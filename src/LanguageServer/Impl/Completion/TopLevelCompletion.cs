    ...
    internal static class TopLevelCompletion {
        public static CompletionResult GetCompletions(Node statement, IScopeNode scopeNode, CompletionContext context) {
            ...
            if (finder.GetExpression(context.Position) is CallExpression callExpr && callExpr.GetArgumentAtIndex(context.Ast, context.Position, out _)) {
                using (context.Analysis.ExpressionEvaluator.OpenScope(context.Analysis.Document, scopeNode)) {
            ...
            var keywords = GetKeywordItems(context, options, scopeNode).ToArray();
