...
namespace Microsoft.PythonTools.Interpreter.Ast {
    class AstAnalysisWalker : PythonWalker {
        ...
        private IMember Clone(IMember member) =>
            member is IMultipleMembers mm ? AstPythonMultipleMembers.Create(mm.Members) :
            member;
        ...
        private IPythonFunctionOverload CreateFunctionOverload(NameLookupContext funcScope, FunctionDefinition node) {
            ...
            var overload = new AstPythonFunctionOverload(funcScope, node.NameExpression.Name, parameters, funcScope.GetLocOfName(node, node.NameExpression));
        ...
        public override bool Walk(ClassDefinition node) {
            ...
            AstPythonType t = member as AstPythonType ??
                (member as IMultipleMembers)?.Members.OfType<AstPythonType>().FirstOrDefault(pt => pt.StartIndex == node.StartIndex);
            if (t == null) {
                t = CreateType(node);
